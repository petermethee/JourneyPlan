...
export interface IElectronAPI {
  ...
  //global
  getAllItems: (tableName: EArtifactTableName, tripId: number) => Promise<IArtifact[]>
  insertItem: (
    tableName: EArtifactTableName,
    item: IArtifact
  ) => Promise<{ id: number; newAttachments: IAttachment[] }>
  updateItem: (tableName: EArtifactTableName, item: IArtifact) => Promise<IAttachment[]>
  ...
  insertArtifactPlanning: (planningArtifact: IPlanningArtifact) => Promise<number>
  updateArtifactPlanning: (planningArtifact: IPlanningArtifact) => Promise<void>
  deleteArtifactPlanning: (planningArtifactId: number) => Promise<void>
  exportAttachments: (planningArtifactId: number) => Promise<boolean>
}
