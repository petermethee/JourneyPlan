...
export const getAllPlannings = createAsyncThunk('getAllPlannings', async (tripId: number) => {
  return await getAllPlanningsAPI(tripId)
})

export const insertPlanning = createAsyncThunk('insertPlanning', async (planning: IPlanning) => {
  const planningId = await insertPlanningAPI(planning)
  return { ...planning, id: planningId }
})

export const updatePlanning = createAsyncThunk('updatePlanning', async (planning: IPlanning) => {
  await updatePlanningAPI(planning)
  return planning
})

export const deletePlanning = createAsyncThunk('deletePlanning', async (planningId: number) => {
...
export const insertArtifactPlanning = createAsyncThunk(
  ...
  async (planningArtifact: IPlanningArtifact) => {
    const planningArtifactId = await insertArtifactPlanningAPI(planningArtifact)
...
export const exportAttachments = createAsyncThunk(
  'exportAttachments',
  async (planningArtifactId: number) => {
    return await exportAttachmentsAPI(planningArtifactId)
  }
)
//#endregion
...
export const planningSlice = createSlice({
  ...
  reducers: {
    ...
    setArtifactIsDragged: (state: PlanningState, action: PayloadAction<EArtifact | null>) => {
      state.artifactIsDragged = action.payload
    },
    addArtifactPlanning: (state: PlanningState, action: PayloadAction<IPlanningArtifact>) => {
      state.planningArtifacts.push(action.payload)
    },
    moveArtifactPlanning: (state: PlanningState, action: PayloadAction<IPlanningArtifact>) => {
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(deleteArtifactPlanning.fulfilled, (state, action) => {
        state.planningArtifacts = state.planningArtifacts.filter((PA) => PA.id !== action.payload)
      })
...
export const selectAllPlannings = (state: RootState) => state.planningReducer.plannings
export const selectPlanningId = (state: RootState) => state.planningReducer.selectedPlanningId
export const selectPlanningArtifacts = (state: RootState) => state.planningReducer.planningArtifacts
export const selectArtifactIsDragged = (state: RootState) => state.planningReducer.artifactIsDragged
