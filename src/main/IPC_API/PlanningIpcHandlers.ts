...
import IPlanningArtifact, { IPlanning } from '../../../src/renderer/src/Models/IPlanningArtifact'
...
export default class PlanningIpcHandlers {
  ...
  initIPCHandlers = () => {
    ipcMain.handle(EIpcChanels.getAllPlannings, async (_event, tripId: number) => {
      return await this.planningsManager.getAllPlannings(tripId)
    })

    ipcMain.handle(EIpcChanels.insertPlanning, async (_event, planning: Partial<IPlanning>) => {
      //planning is partial to allow id deletion
      return await this.planningsManager.insertPlanning(planning)
    })

    ipcMain.handle(EIpcChanels.updatePlanning, async (_event, planning: IPlanning) => {
      await this.planningsManager.updatePlanning(planning)
    })

    ipcMain.handle(EIpcChanels.deletePlanning, async (_event, planningId: number) => {
      await this.planningsManager.deletePlanning(planningId)
    })

    ipcMain.handle(EIpcChanels.getAllArtifactsPlanning, async (_event, planningId: number) => {
    ...
    ipcMain.handle(
      ...
      async (_event, planningArtifact: IPlanningArtifact) => {
        return await this.planningsManager.insertArtifactPlanning(planningArtifact)
      }
    )
    ipcMain.handle(
    ...
    ipcMain.handle(EIpcChanels.exportAttachments, async (_event, planningId: number) => {
