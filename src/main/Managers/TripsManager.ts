...
export default class TripsManager {
  ...
  insertTrip = async (trip: Partial<ITrip>) => {
    if (trip.image_path) {
      ...
      const newPath = join(tripImagesPath, fileName)

      try {
        copyFileSync(trip.image_path, newPath)
    ...
    let sql = 'INSERT INTO ' + TablesName.trips + columns + ' VALUES ' + placeholders
  ...
  updateTrip = async (trip: ITrip) => {
    if (trip.image_path) {
      ...
      try {
        ...
        unlinkSync(imagePath.image_path)
      } catch (error) {
  ...
  deleteTrip = async (tripId: number) => {
    ...
    const paths = [...activitesPaths, ...transportsPaths, ...accommodationsPaths]
